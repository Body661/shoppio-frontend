...
const OrderCard = ({order}) => {
    ...
    return (<Link to={order?._id} className="user-order mt-4">
        <Row>
            <div className="xs-black-text">{formatDate(order?.createdAt)} | Order ID: {order?._id}</div>
        </Row>
            ...
            {order?.cartItems && (order?.cartItems?.map((item, index) => {
                return (<Row className="d-flex align-items-center">
                        ...
                        <Col xs="9" className="d-flex flex-column align-items-start" style={{
                            ...
                            <span>
                                    {order?.isDelivered ? "Delivered" : "Not delivered"} | {order?.isPaid ? "Paid" : "Not paid"} | {order?.paymentMethodType === "card" ? "Card" : "Cash"}
                                </span>
                        </Col>
                    </Row>)
            }))}
